...
class ADMM_Net(nn.Module):
    def __init__(
        ...
        # ).unsqueeze(0)
        self.h_complex = torch.complex(self.h_var, self.h_zeros).unsqueeze(0)

        self.H = nn.Parameter(
            torch.fft.fft2(torch.fft.ifftshift(self.h_complex).squeeze()),
        ...
        # )
        self.Hconj = nn.Parameter(torch.conj(self.H), requires_grad=False)

        self.HtH = nn.Parameter(self.H * self.Hconj,
                                requires_grad=False
        )
