...
def admm(model, in_vars, alpha2k_1, alpha2k_2, CtC, Cty, mu_auto, n, y):
    ...
    primal_resid_w = []
    cost = []
    # print(model.HtH.device, model.LtL.device)
    Smult = 1 / (
        mu1 * model.HtH.real + mu2 * model.LtL + mu3
    ...
    if "prox" in model.learning_options["learned_vars"]:  # use learned prox
        ...
        symm = []
        SKP_numerator = make_complex(skp_numerator)
        Smult = make_complex(Smult)

        # FFT of the numerator
        SKP_numerator_fft = torch.fft.fft2(SKP_numerator)

        # Frequency domain multiplication
        product = complex_multiplication(Smult, SKP_numerator_fft)

        # IFFT and extract the real part
        skp = make_real(torch.fft.ifft2(product))
