...
def load_sim_save(simulator, obj_path, save_path, use_adjust_light_intensity=False):
    ...
    cv2.imwrite(save_png_path.replace(".npy", ".png"), img)
...
if __name__ == "__main__":
    ...
    # transfer the psf 
    simulator = FarFieldSimulator(object_height = 0.4, scene2mask = 0.0868, mask2sensor = 2e-3, sensor = sensor, psf = psf, is_torch=True, quantize=False, return_float=True)
...
#python tools/sim_capture_diff.py --obj_path /root/caixin/flatnet/data/diffusercam/ground_truth_lensed --save_path /root/caixin/flatnet/data/diffusercam/sim_captures --psf_path /root/caixin/flatnet/data/diffusercam/psf.tiff