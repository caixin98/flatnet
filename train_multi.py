            ...
            for i, batch in enumerate(data.train_loader):
                ...
                if epoch >= args.fft_epochs:
                    ...
                    for i in range(args.multi):
                        fft_optimizer[i].zero_grad()
                        g_loss.fft_loss[i].backward(retain_graph=True)
                        if args.distdataparallel:
                            saved_fft_gard.append(
                                FFT.module.wiener_crop[i].grad.clone()
                            )
                        else:
                            saved_fft_gard.append(FFT.wiener_crop[i].grad.clone())
                        fft_lr_scheduler[i].step(
                                epoch - args.fft_epochs + i / len(data.train_loader)
                            )   
                g_optimizer.zero_grad()
                g_loss.total_loss.backward()
                g_optimizer.step()
                # recover the gradient of fft
                if epoch >= args.fft_epochs:
                    for i in range(args.multi):
                        if args.distdataparallel:
                            FFT.module.wiener_crop[i].grad += args.grad_lambda * saved_fft_gard[i]
                        else:
                            FFT.wiener_crop[i].grad += args.grad_lambda * saved_fft_gard[i]
                        fft_optimizer[i].step()
